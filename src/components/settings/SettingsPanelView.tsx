    ...
    <div className="min-h-full bg-[#0b0c10] pb-24">
              ...
              <div className="flex rounded-lg overflow-hidden border border-white/10 bg-black/20 p-0.5">
                <button type="button" onClick={() => { setAccountMode('demo'); updateUserSettings({ defaultAccountMode: 'demo' }); }}
                  className={`px-3 py-1 text-[10px] font-black uppercase rounded-md transition-all ${
                    accountMode === 'demo' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'
                  }`}>Demo</button>
                <button type="button" onClick={() => { setAccountMode('real'); updateUserSettings({ defaultAccountMode: 'real' }); }}
                  className={`px-3 py-1 text-[10px] font-black uppercase rounded-md transition-all ${
                    accountMode === 'real' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'
                  }`}>Real</button>
              </div>
            }
          />
      ...
      {editNameOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-4"
          ...
          <div className="w-full max-w-sm bg-[#14161f] border border-white/10 rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-4">
            ...
            <input value={nameInput} onChange={e => setNameInput(e.target.value)} placeholder="Your display name"
              className="w-full bg-[#0b0c10] border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm font-medium outline-none focus:border-amber-500/50 transition-colors placeholder:text-gray-600"
